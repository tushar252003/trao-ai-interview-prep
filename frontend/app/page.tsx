...
export default function Home(){
  ...
  return <main className="min-h-screen bg-slate-50">
    ...
    <div className="mx-auto max-w-7xl p-5">
      ...
      {!selected&&<section className="mt-8 rounded-2xl bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">Batch entry</h2><p className="text-sm text-slate-500">Upload a JSON array of {`{ id, jd, company_url, days }`} cases and generate them through the same API pipeline.</p></div><input type="file" accept="application/json,.json" onChange={async e=>{const f=e.target.files?.[0];if(!f)return;try{const cases=JSON.parse(await f.text());if(!Array.isArray(cases))throw new Error("JSON must be an array");for(const c of cases){const {jobId}=await api("/api/kits/generate",{method:"POST",body:JSON.stringify({jd:c.jd,company_url:c.company_url,days:Number(c.days)})});let done=false;while(!done){await new Promise(r=>setTimeout(r,800));const d=await api(`/api/jobs/${jobId}`);if(d.job.status==="complete"){await api("/api/kits/save-generated",{method:"POST",body:JSON.stringify({input:{jd:c.jd,company_url:c.company_url,days:Number(c.days)},kit:d.job.kit})});done=true}else if(d.job.status==="failed")throw new Error(`${c.id||"case"}: ${d.job.error?.message||"failed"}`)}}setKits((await api("/api/kits")).kits);alert("Batch cases generated.")}catch(err:any){alert(err.message)}}} className="max-w-xs text-sm" /></div></section>}
